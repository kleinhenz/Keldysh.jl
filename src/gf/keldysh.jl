struct KeldyshTimeGF{T, scalar} <: AbstractTimeGF{T}
  ...
  function KeldyshTimeGF(grid::TimeGrid,
                      gtr::AntiHermitianStorage{T,scalar},
                      les::AntiHermitianStorage{T,scalar}) where {T, scalar}
...
function KeldyshTimeGF(::Type{T}, grid::TimeGrid, norb=1, scalar=false) where T <: Number
  nt = length(grid, forward_branch)

  gtr = AntiHermitianStorage(T, nt, norb, scalar)
  les = AntiHermitianStorage(T, nt, norb, scalar)

  KeldyshTimeGF(grid, gtr, les)
end
KeldyshTimeGF(grid::TimeGrid, norb=1, scalar=false) = KeldyshTimeGF(ComplexF64, grid, norb, scalar)
...
function KeldyshTimeGF(f::Function, ::Type{T}, grid::TimeGrid, norb=1, scalar=false) where T <: Number
  G = KeldyshTimeGF(T, grid, norb, scalar)
  D = TimeDomain(G)
...
KeldyshTimeGF(f::Function, grid::TimeGrid, norb=1, scalar=false) = KeldyshTimeGF(f, ComplexF64, grid, norb, scalar)
...
function KeldyshTimeGF(dos::AbstractDOS, β, grid::TimeGrid)
  ...
  KeldyshTimeGF(grid,1,true) do t1, t2
    G[t1,t2]
  end
end
